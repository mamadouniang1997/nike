...
import { setAddtemToCart, setOpenCart } from '../../app/CartSlice.js';
     ...
     ${color} ${shadow}grid items-center ${
      ...
      duration-700 ease-in-out w-full hover:scale-105`}>
        <div className={`grid items-center ${
            ...
            <p className='text-slate-200text-slate-200 filter drop-shadow text-base md:text-sm font-normal'>{text}</p>
            ...
            <div className='flex items-center gap-3'>
                <button type='button' className='bg-white/90 blur-effect-theme
                 button-theme p-0.5 shadow shadow-sky-200 ' onClick={()=> onAddToCart()} >
                    <ShoppingBagIcon className='icon-style text-slate-900 '/></button>
                <button type='button' className='bg-white/90 blur-effect-theme button-theme px-2 
                py-1 shadow shadow-sky-200 text-sm
                 text-black'  onClick={()=> {onAddToCart(); onCartToggle();}}> {btn} </button>
            </div>
        </div>
